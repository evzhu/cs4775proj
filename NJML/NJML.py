...
def neighbor_joining_root_fix(distance_matrix):
    ...
    # Convert distance matrix to numerical form
    ...
    # Do not add a branch length for this connection to avoid root branch length
...
def final_generate_newick_ignore_root_length(tree, lengths, root=None):
    ...
    def dfs(node, parent):
        ...
        if not children:
            ...
            if parent:  # Internal node
                branch_length = lengths.get((node, parent), lengths.get((parent, node), 0.0))
                return f"({','.join(branches)}):{branch_length:.6f}"
            else:  # Root node
                return f"({','.join(branches)})"  # Explicitly omit branch length for the root
